...
*Threshold BLS beacon*​[near.org](https://near.org/blog/randomness-in-blockchain-protocols) | Produces truly *unbiased, unpredictable* outputs via threshold BLS (acting as a VRF). Well-studied in production design. It achieves a single collective signature per round. | Requires expensive setup and coordination (large n-of-k DKG across nodes)​. The fixed committee model incurs latency and complexity. Without an additional VDF layer, it depends on at least one honest node for unpredictability. |
| **NEAR Protocol** 
